...
def render(rows, source):
    ...
    shapes = sorted(stats, key=lambda s: -s[0])
    ...
    for shape in shapes:
        ...
        label = (f'<h3>d = {dim} <span>&nbsp;·&nbsp; {anchors}+{cands} points'
                 f'</span></h3>')
    ...
    tables = "".join(
        f'<section><h3>d = {d} &nbsp;·&nbsp; {a}+{c} points</h3>'
...
<footer>
  ...
  <code>{html.escape(os.path.basename(source))}</code>. Reproduce with
...
if __name__ == "__main__":
    ...
    with open(args.out, "w") as fh:
        fh.write(render(rows, args.json))
